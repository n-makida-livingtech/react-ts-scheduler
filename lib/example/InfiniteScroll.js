...
var InfiniteScroll = (function (_super) {
    ...
    function InfiniteScroll(props) {
        ...
        _this.onScrollRight = function (schedulerData, schedulerContent, maxScrollLeft) {
            if (schedulerData.viewType === Scheduler_1.SchedulerViewTypes.Month) {
                schedulerData.next();
        ...
        _this.onScrollLeft = function (schedulerData, schedulerContent, maxScrollLeft) {
            if (schedulerData.viewType === Scheduler_1.SchedulerViewTypes.Month) {
                schedulerData.prev();
                schedulerData.setEvents(DemoData_1.DemoData.events);
                _this.setState({
        ...
        var schedulerData = new Scheduler_1.SchedulerData(ExampleFunction.updateSchedulerDataState.bind(_this), ExampleFunction.getNow(), Scheduler_1.SchedulerViewTypes.Month, false, false, {
            views: [
                { viewName: 'Month', viewType: Scheduler_1.SchedulerViewTypes.Month, showAgenda: false, isEventPerspective: false },
            ],
        });
        moment.locale('en');
