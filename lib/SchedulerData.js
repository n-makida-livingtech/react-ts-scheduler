        ...
        slots.forEach(function (slot) {
            ...
            var slotRenderData = {
                ...
                rowHeight: _this.config.nonAgendaSlotMinHeight !== 0
                    ? _this.config.nonAgendaSlotMinHeight
                    : _this.config.eventItemLineHeight + 2,
    ...
    SchedulerData.prototype._createRenderData = function () {
                    ...
                    if (headerEnd > eventStart_1 && headerStart < eventEnd_1) {
                        ...
                        if (header.count > resourceEvents_1.rowMaxCount) {
                            ...
                            var newRowHeight = rowsCount * _this.config.eventItemLineHeight +
                                (_this.config.creatable && _this.config.checkConflict === false ? 2 : 2);
                        }
                        if (pos_1 === -1) {
                            var tmp = 0;
        ...
        if (cellMaxEventsCount <= cellMaxEventsCountValue || this.behaviors.getSummaryFunc !== undefined) {
            initRenderData.forEach(function (resourceEvents) {
                ...
                if (hasSummary) {
                    var rowsCount = cellMaxEventsCount <= cellMaxEventsCountValue && resourceEvents.rowMaxCount > cellMaxEventsCount
                        ? cellMaxEventsCount
                        : resourceEvents.rowMaxCount;
                    var newRowHeight = (rowsCount + 1) * _this.config.eventItemLineHeight +
                        (_this.config.creatable && _this.config.checkConflict === false ? 20 : 2);
