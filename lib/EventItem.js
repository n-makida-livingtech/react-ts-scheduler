...
var EventItem = (function (_super) {
            ...
            if (config.checkConflict) {
                ...
                events.forEach(function (e) {
                    if (schedulerData.getEventSlotId(e) === slotId && e.id !== eventItem.id) {
                        var eStart = moment(e.start);
                        var eEnd = moment(e.end);
                        if ((start_2 >= eStart && start_2 < eEnd) ||
                            (end_2 > eStart && end_2 <= eEnd) ||
                            (eStart >= start_2 && eStart < end_2) ||
                            (eEnd > start_2 && eEnd <= end_2)) {
                            hasConflict = true;
                        }
                    }
                });
    ...
    EventItem.prototype.render = function () {
        ...
        return isDragging ? null : schedulerData.isResizing() ||
            ...
            eventItem.showPopover === false ? (React.createElement("div", null, connectDragPreview(connectDragSource(tt)))) : (React.createElement(EventItemPopover_1.default, __assign({}, this.props, { eventItem: eventItem, title: eventItem.title, startTime: eventItem.start, endTime: eventItem.end, connectDragSource: connectDragSource, connectDragPreview: connectDragPreview, timelineEvent: tt })));
    };
    return EventItem;
}(react_1.Component));
