...
export const EventComponent: React.FC<EventProps> = (props) => {
  ...
  if (!!props.eventItem.type) {
    borderColor =
      props.eventItem.type == 1 ? 'rgba(0,139,236,1)' : props.eventItem.type == 3 ? 'rgba(245,60,43,1)' : '#999';
    backgroundColor = props.eventItem.type == 1 ? '#80C5F6' : props.eventItem.type == 3 ? '#FA9E95' : '#D9D9D9';
  }
  let divStyle = {
    borderLeft: borderWidth + 'px solid ' + borderColor,
    backgroundColor,
    height: props.mustBeHeight,
    maxWidth: undefined,
  };
  ...
  return (
    <div key={props.eventItem.id} className={props.mustAddCssClass} style={divStyle}>
  ...
  const eventItemTemplate = (
    <div
      ...
      <span style={{ marginLeft: '10px', lineHeight: `${config.eventItemHeight}px` }}>
        {isInPopover ? `${eventItem.start.format('HH:mm')} ${titleText}` : titleText}
      </span>
    </div>
  );
